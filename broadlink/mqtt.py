...
def on_message(client, device, msg):
    ...
    try:
        ...
        logging.debug("Received MQTT message " + msg.topic + " " + action+" Command: %s Device_Type: %s " % (command,device.type))
        ...
        if device.type == 'RM2' or device.type == 'RM4':
            file = dirname + "commands" 
            handy_file = file + '/' + action
            print("File %s" % handy_file)

            if command == 'macro':
                file = dirname + "macros/" + action
                macro(device, file)
                return
            elif command == '' or command == 'auto':
                ...
                record(device, handy_file)
                return
            elif command == 'recordrf':
